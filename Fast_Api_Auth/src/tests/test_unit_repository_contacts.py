...
class TestNotes(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_contact_by_name(self):
        ...
        result = await get_contact_by_name(contact_name='Andriy', db=self.session)
        self.assertEqual(result, contact)

    async def test_get_contact_by_surname(self):
        contact = [Contact()]
        self.session.query().filter_by().all.return_value = contact
        result = await get_contact_by_surname(contact_surname='Martynyuk', db=self.session)
        self.assertEqual(result, contact)

    async def test_create_contact(self):
        body = ContactModel(name="Andriy",
                            surname="Martynyuk",
