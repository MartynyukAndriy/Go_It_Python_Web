...
if __name__ == "__main__":
    ...
    name1 = Name("Andriy")
    phone1 = Phone("0991113456")
    address = Address("IF")
    birthday = Birthday("10.10.1998")
    email = Email("Andriy@gmail.com")
    name2 = Name("Igor")
    phone2 = Phone("0897654534")
    rec1 = Record(name1, phone1, address, birthday, email)
    rec2 = Record(name2, phone2)
    addressbook.add_record(rec1)
    addressbook.add_record(rec2)
    print(addressbook.show_records())
    print(addressbook.show_record("Andriy"))

    notebook = NoteBook()
    notename1 = NoteName("Beer")
    note1 = Notes("Buy a 2 beers")
    tag = Tags("2 beers")
    notename2 = NoteName("Notebook")
    note2 = Notes("Buy a new notebook")
    notes1 = RecordNote(notename1, note1, tag)
    notes2 = RecordNote(notename2, note2)
    notebook.add_note(notes1)
    notebook.add_note(notes2)
    print(notebook.show_record("Beer"))
    print(notebook.show_records())
